...
def move2board(y): 
    ...
    '''
    return Vec({'y1','y2','y3'}, {'y1': y.f['y1']/y.f['y3'], 'y2': y.f['y2']/y.f['y3'], 'y3': 1})
...
def make_nine_equations(corners):
    ...
    return [item for sublist in [make_equations(x1,x2,w1,w2) for (x1,x2,w1,w2) in {(x1,x2,w12[ind][0], w12[ind][1]) for ind,(x1,x2) in enumerate(corners)}] for item in sublist] + [w]
...
# # 5.12.5
# # (X_pts, colors) = file2mat('./part5_The_Basis/board.png', ('x1','x2','x3'))
...
def mat_move2board(Y):
    ...
    return Mat(Y.D, {(k1,k2):Y.f[(k1,k2)]/Y.f[('y3',k2)] for (k1,k2) in Y.f})
