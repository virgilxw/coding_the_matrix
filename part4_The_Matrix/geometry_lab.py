...
def rotation(theta):
    ...
    return Mat(({'x','y','u'}, {'x','y','u'}), {('x','x'):cos(theta), ('x','y'):-sin(theta), ('y','x'): sin(theta), ('y','y'): cos(theta), ('u', 'u'): 1})
...
def grayscale():
    ...
    '''
    return Mat(({'r', 'g', 'b'}, {'r', 'g', 'b'}), {('r', 'r'): 77/256, ('g', 'g'): 151/256, ('b', 'b'): 28/256})
...
def reflect_about(x1, y1, x2, y2):
    ...
    return  translation(x1,y1) * rotation(atan((y2-y1)/(x2-x1))) * reflect_x() * rotation(-atan((y2-y1)/(x2-x1))) *  translation(-x1,-y1)
