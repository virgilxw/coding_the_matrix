...
def find_error(syndrome):
    ...
    Hcd = mat2coldict(H)
    return Vec({0, 1, 2, 3, 4, 5, 6}, {c:one for c in Hcd if Hcd[c] == syndrome})
...
def find_error_matrix(S):
    ...
    Srd = mat2coldict(S)
    return coldict2mat({k:find_error(Srd[k]) for k in Srd})
...
E = noise(P, 0.02)
corrupted_mat = E+P
corrupted_text = bits2str(mat2bits(corrupted_mat))

## Task 9
C = G*P
bits_before = len(mat2bits(corrupted_mat))
bits_after = len(mat2bits(C))

## 4.14.13
CTILDE = corrupted_mat
bits2str(mat2bits(CTILDE))
...
def correct(A):
    ...
    Example:
        ...
        True
    """
    error_vector = find_error_matrix(H*A)
    code_word = error_vector + A
    return code_word
