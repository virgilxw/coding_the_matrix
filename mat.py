...
def equal(A, B):
    ...
    return mat2rowdict(A) == mat2rowdict(B)
...
def add(A, B):
    ...
    for r in itertools.product(A.D[0], A.D[1]):
        x = A[r] + B[r]
...
def scalar_mul(M, x):
    ...
    return Mat((M.D), {(p, q): v * x for (p,q),v in M.f.items()})
...
def matrix_matrix_mul(A, B):
    ...
    from matutil import mat2rowdict, mat2coldict, rowdict2mat
    import itertools
